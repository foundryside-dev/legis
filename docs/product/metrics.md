...
| **Cross-tool rename-parser conformance** — the rename feed's contract is pinned by shared **two-way vectors**, not a legis-authored parser mock | must hold (binary) | ⚠ **BREACHED — target of the Now bet (PDR-0011).** `/git/renames` (consumed by Loomweave *today*) + `/git/rename-feed` assert against `_parse_like_loomweave`, legis's own guess of Loomweave's Rust `parse_legis_rename_json`, so real drift would stay green. Closes when the mock is replaced by a committed two-way vector file + that file is handed to Loomweave. | 2026-07-01 |
