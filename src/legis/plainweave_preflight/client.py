...
]  # returns the parsed tool result (validated below)
    ...
    *,
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise PlainweaveError(
            f"{tool} data.scope.requirement_page.{field} must be an integer"
        )
    if value < minimum or (maximum is not None and value > maximum):
        raise PlainweaveError(
            f"{tool} data.scope.requirement_page.{field} is out of bounds"
        )
    return value
...
def _validate_requirement_page(
    ...
    *,
    ...
    if not isinstance(scope, dict):
        if required:
            raise PlainweaveError(
                f"{tool} data.scope.requirement_page is required for continuation"
            )
        return
    ...
    if limit != effective_limit:
        raise PlainweaveError(
            f"{tool} data.scope.requirement_page.limit does not match the request"
        )
    if offset != effective_offset:
        raise PlainweaveError(
            f"{tool} data.scope.requirement_page.offset does not match the request"
        )
    ...
    if (
        ...
        raise PlainweaveError(
            f"{tool} data.scope.requirement_page.returned does not match requirement_ids"
        )
    expected_returned = min(limit, max(total - offset, 0))
    if returned != expected_returned:
        raise PlainweaveError(
            f"{tool} data.scope.requirement_page.returned is inconsistent"
        )
    ...
    if not isinstance(has_more, bool) or has_more != expected_has_more:
        raise PlainweaveError(
            f"{tool} data.scope.requirement_page.has_more is inconsistent"
        )
    ...
    if not next_valid or next_offset != expected_next:
        raise PlainweaveError(
            f"{tool} data.scope.requirement_page.next_offset is inconsistent"
        )
    ...
    def _call(
        ...
        *,
        ...
        if not isinstance(env, dict):
            raise PlainweaveError(
                f"{tool} returned {type(env).__name__}, expected an envelope object"
            )
        if env.get("schema") != schema:
            raise PlainweaveError(
                f"{tool} returned schema {env.get('schema')!r}, expected {schema!r}"
            )
        ...
        if not isinstance(data, dict):
            raise PlainweaveError(
                f"{tool} envelope data is {type(data).__name__}, expected an object"
            )
        boundary = data.get("authority_boundary")
        if not isinstance(
            boundary, dict
        ):  # malformed boundary fails closed (GV-LG-3 input)
            raise PlainweaveError(
        ...
        if (
            boundary.get("live_peer_calls") is not False
        ):  # a live peer call is a side effect (GV-LG-3)
            raise PlainweaveError(
                f"{tool} authority_boundary.live_peer_calls is not false: {boundary.get('live_peer_calls')!r}"
            )
        if (
            boundary.get("governance_verdicts") is not False
        ):  # this producer must NEVER claim a verdict (GV-LG-3)
            raise PlainweaveError(
                f"{tool} authority_boundary.governance_verdicts is not false: "
                f"{boundary.get('governance_verdicts')!r}"
            )
        if (
            "freshness" not in data or "facts" not in data
        ):  # degraded -> unavailable, not bare empty 'checked'
            raise PlainweaveError(
...
def _read_jsonrpc_result(stdout_text: str, response_id: int) -> dict:
    for line in stdout_text.splitlines():
        ...
        try:
            ...
            raise PlainweaveError(
                f"plainweave-mcp emitted a non-JSON line: {exc}"
            ) from exc
        if isinstance(msg, dict) and msg.get("id") == response_id:
            if "error" in msg:
                raise PlainweaveError(
                    f"plainweave-mcp returned a JSON-RPC error: {msg['error']}"
                )
            result = msg.get("result")
            if not isinstance(result, dict):
                raise PlainweaveError(
                    f"plainweave-mcp result is {type(result).__name__}, expected an object"
                )
            return result
    raise PlainweaveError(
        f"plainweave-mcp produced no JSON-RPC response for id={response_id}"
    )
...
class StdioMcpInvoke:
    ...
    def __call__(self, tool: str, arguments: dict) -> dict:
        ...
        msgs = (
            {
                ...
                "params": {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "legis", "version": "1"},
                },
            },
        ...
        try:
            ...
            raise PlainweaveError(
                f"plainweave-mcp spawn/timeout failed: {exc}"
            ) from exc
        ...
        try:
            result = _read_jsonrpc_result(
                proc.stdout.decode("utf-8", "replace"), response_id=2
            )
            if result.get("isError"):
                raise PlainweaveError(
                    f"plainweave tool {tool} returned an error result (rc={proc.returncode}, stderr={err!r})"
                )
            ...
            raise PlainweaveError(
                f"plainweave tool {tool} result had no usable envelope (rc={proc.returncode}, stderr={err!r})"
            )
        except PlainweaveError:
            raise
        except Exception as exc:  # ANY parse fault fails closed
            raise PlainweaveError(
                f"plainweave tool {tool} result parse failed: {exc} (rc={proc.returncode}, stderr={err!r})"
            ) from exc
